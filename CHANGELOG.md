...
## Companion update — `ENFORCEABILITY-DISCIPLINE.md` v0.3.0 — 2026-06-04

Second stress-test repair. Running differential privacy (`(ε,δ)`-DP libraries) through the B5 classification showed the contract's spine held — DP stayed out of Class A on the correct grounds (a bounded violation is still a representable computation, not unrepresentability), and R4 forced ε disclosure — but its **Class B vocabulary broke**: B was described in purely conformal (average-case) terms ("coverage, contingent on exchangeability, which shift breaks"), which mis-seams every worst-case bound. DP is a Class B guarantee that is worst-case over inputs, *survives* distribution shift, and fails instead at a chosen parameter. Repairs: **split Class B into B-avg (conformal/OOD — dies under shift) and B-wc (DP, crypto reductions — survives shift, dies at its parameter)** in the A2 table; named the **policy seam** (a free parameter that can be technically valid yet vacuous); added **R7** (a parameterized guarantee discloses its parameter — ε, α, confidence level — at guarantee-altitude; the B-class analog of R4). Added DP as a B5 worked example, relabeled the existing B entries B-avg, and updated the B4 scorecard examples. `UNTRUST.md` unchanged (still v2.0.1).

## Companion update — `ENFORCEABILITY-DISCIPLINE.md` v0.2.0 — 2026-06-04

Stress-test repair. Running OpenAI Structured Outputs (strict JSON-schema constrained decoding) through the B5 four-field classification surfaced a gap the v0.1.0 single-class schema could not express: one property (schema conformance) is Class A *per completed call* but Class B *over the call distribution*, because completion is input-dependent (truncation/refusal). Repairs: added **R6** (class is stated per input-slice when it varies); upgraded the B4 scorecard from a `(letter, condition)` pair to a `(letter, condition, coverage)` **triple** — a bare condition was itself flattening a distribution, the same failure B4 names, one level down; allowed the B1 **Seam** field to carry a *list* (real tools have seam stacks — e.g. schema fidelity + decoder correctness). Added Structured Outputs as a second worked example in B5, lifted the existing class cells to triple form, and corrected the table's constrained-decoder row (was over-claimed as `(A, unconditional for syntax)`). Refined A1 to state that class is a function of property, mechanism, *and* input-slice. `UNTRUST.md` unchanged (still v2.0.1).

## Companion added — `ENFORCEABILITY-DISCIPLINE.md` v0.1.0 — 2026-06-04

Repo-level addition, not a `UNTRUST.md` version change — the document stays at v2.0.1, unedited. Added a companion conformance contract that externalizes the §12 enforceability cut and turns it into a discipline binding on trust *claims*. Two layers: **Part A** (argument) — a property's enforceability class is fixed by what a claim is *pointed at*, not by its mechanism; the three classes restated with each one's seam made explicit (labeling / autoformalisation / source-trust); the three claim-layer erosion vectors (marketing gradient, aggregation halo, scope creep). **Part B** (discipline) — a four-field labeling contract (Property / Class / Seam / Out-of-scope), five conformance rules, a failure-vector→clause map, a conditional-preserving scorecard (`class` is a `(letter, condition)` pair, never bare), and worked classifications of five example tools. Defers to UNTRUST §12 as the source of record. Also added a `README.md` Contents pointer. The companion carries its own version line (v0.1.0); its history will be tracked here under this companion heading, kept separate from the `UNTRUST.md` version entries below.

## v2.2.0 — 2026-06-04

Added a use-caution to §16 (Notes on use): the enforceability classes are a **lens**, and reifying them into a voluntary discipline (a conformance standard, labeling contract, or checklist) inherits no enforcement — any actor can decline it, so by the §2 criterion such a standard is **Class C, a mitigation, not a fix**, until something external (adoption, contract, audit, law) enforces it. This is the §9 trusted-base lesson turned on the frame itself; it forecloses the drift of mistaking "applying the classes" for a fix. Closes the cross-reference asymmetry with a parenthetical pointer to the `ENFORCEABILITY-DISCIPLINE.md` companion (which labels itself Class C accordingly). No claim in §0–§15 changes; the §2 criterion, the A/{B,C} line, and scope are unchanged. Surfaced by the companion's own "binds nobody" admission.

## v2.1.0 — 2026-06-04

Refinement to the §12 enforceability taxonomy: split Class B into **B-avg** (average-case: conformal, bounded OOD — rests on exchangeability, dies under shift) and **B-wc** (worst-case: differential privacy, randomized-smoothing certificates — hold over all inputs, survive shift, fail instead at a chosen parameter such as ε or a confidence level). The old single Class B was described in purely conformal terms, which mis-seamed every worst-case bound. Propagated the split into §13's robustness/OOD cluster — the one Part II cluster where both species appear (conformal = B-avg, randomized smoothing = B-wc). Added references [36] (Dwork et al. 2006; floating-point seam, Mironov 2012) and [37] (Cohen et al. 2019). The A-vs-{B,C} line, the §2 criterion, the four sketches + Pattern 5, and document scope are unchanged. Surfaced by stress-testing the `ENFORCEABILITY-DISCIPLINE.md` companion against differential privacy.
